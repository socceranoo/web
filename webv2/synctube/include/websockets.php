...
// todo: if($numBytes === false) { error handling } elseif ($numBytes === 0) { remote client disconected }
	...
	abstract protected function getSessionForSessionId($id);
	...
	protected function broadcast_except_sender($sender, $message) {
		$session_obj = $sender->session_obj;
		foreach ($session_obj->long_user_array as $k ) {
	...
	protected function broadcast_to_multiple_signins($sender, $message) {
		foreach ($this->users as $k ) {
			if ($k->socket != $this->master && $k != $sender) {
				if ($k->username == $sender->username)
					$this->send($k, $message);
			}
		}
	}
	...
	protected function connect($socket) {
		$id = uniqid();
		$user = new WebSocketUser($id,$socket);
		$user->username="null";
	...
	protected function disconnect($socket,$triggerClosed=true) {
		...
		if ($foundUser !== null) {
			//print "$disconnectedUser->id\n";
			$message = "REMOVEUSER:".$disconnectedUser->id;
			$this->broadcast_except_sender($disconnectedUser, $message);
			$session_obj = $disconnectedUser->session_obj;
			if ($session_obj) {
				$session_obj->removeUser($disconnectedUser);
			}
