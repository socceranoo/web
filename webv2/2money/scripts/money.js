...
function change_ind_amount(event) {
	id=event.target.id;
...
function close_payer_payee(event) {
	id = event.target.id;
...
function select_payment_payer(event) {
	var id = event.target.id;
	var payer = $(this).val();
	if (payer != "default")
		set_payment_payer(payer);
}
function set_payment_payer(payer) {
	payment_payer = payer;
...
function select_payment_payee(event) {
	var id = event.target.id;
	var payee = $(this).val();
	if (payee != "default")
		set_payment_payee(payee);
}
function set_payment_payee(payee) {
	payment_payee = payee;
...
function add_payer(payer, amount) {
...
function select_payee(event) {
	id = event.target.id;
	payee = $(this).val();
	if (payee != "default")
		add_payee(payee);
}
function add_payee(payee, amount) {
...
function common_ajax(file, actionval, actionval2, argval, argval2, argval3) {
	...
	$.post(file, {action:actionval, arg:argval, arg2:argval2, arg3:argval3},function(data){
		var buttonid, buttonclass, buttonvalue, buttonstr;
		...
		switch (actionval2) {
			...
			case "edit-bill-info":
				...
				break;
			case "del-bill-info":
				buttonid = "delbutton"+argval;
				buttonclass ="delbillbutton";
				buttonvalue ="Confirm Delete";
				buttonstr = "<h4><input type=button class="+buttonclass+" id="+buttonid+" value='"+buttonvalue+"'/></h4>";
				document.getElementById("current-content").innerHTML += buttonstr;
				$(".delbillbutton").click(delete_bill);
				break;
			case "rev-bill-info":
				buttonid = "revbutton"+argval;
				buttonclass ="revbillbutton";
				buttonvalue ="Confirm Revival";
				buttonstr = "<h4><input type=button class="+buttonclass+" id="+buttonid+" value='"+buttonvalue+"'/></h4>";
				document.getElementById("current-content").innerHTML += buttonstr;
				$(".revbillbutton").click(revive_bill);
				break;
			case "trash-bill-info":
				buttonid = "trashbutton"+argval;
				buttonclass ="trashbillbutton";
				buttonvalue ="Confirm permanent delete";
				buttonstr = "<h4><input type=button class="+buttonclass+" id="+buttonid+" value='"+buttonvalue+"'/></h4>";
				document.getElementById("current-content").innerHTML += buttonstr;
...
function parse_bill(jsonString) {
	...
	show_ab_form();
	oldbill = true;
	edit_bill_id = parseInt(obj['id'], 10);
